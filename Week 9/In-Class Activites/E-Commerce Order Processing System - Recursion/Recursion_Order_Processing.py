'''
Probelm:
you need to process the most recent orders first to ensure faster delivery for last-minute purchases. The orders are currently stored in a singly linked list, with the head being the first order received. Your task is to reverse the list so the most recent orders are processed first.

Reversal Functionality: Implement the reverse() function to recursively reverse the linked list. This function should modify the links between nodes in the list to invert the order of the nodes. After reversal, the display() function should present the orders starting from the most recently added.
...
#  function to perform the actual reverse_recursive
